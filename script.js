...
function error_img () {
  setTimeout("mjpeg_img.src = PiUrl + 'cam_pic.php?time=' + new Date().getTime();", 100);
}
...
function updatePreview(cycle)
{
   if (mjpegmode)
   {
      ...
      if (previous_halted != halted)
      {
         if(!halted)
         {
            mjpeg_img.src = PiUrl + "cam_pic_new.php?time=" + new Date().getTime() + "&pDelay=" + preview_delay;
         }
         else
         {
